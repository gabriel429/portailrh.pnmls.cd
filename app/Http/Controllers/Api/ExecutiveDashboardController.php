...
class ExecutiveDashboardController extends ApiController
{
    ...
    private function normalizedUserProfile($user): string
    {
        $agent = $user?->agent;

        return trim(implode(' ', array_filter([
            $this->normalizedText($user?->role?->nom_role),
            $this->normalizedText($agent?->fonction),
            $this->normalizedText($agent?->poste_actuel),
    ...
    private function isSelDashboardUser($user): bool
    {
        if (!$user) {
            return false;
        }

        $roleName = $this->normalizedText($user->role?->nom_role);
        if ($roleName === 'sel') {
            return true;
        }

        $agent = $user->agent;
        $organe = $this->normalizedText($agent?->organe);
        $profile = $this->normalizedUserProfile($user);
    ...
    private function isRhLocalDashboardUser($user): bool
    {
        if (!$user) {
            return false;
        }
        ...
        $roleName = $this->normalizedText($user->role?->nom_role);
        if (in_array($roleName, ['rh local', 'aaf local', 'assistant administratif et financier'], true)) {
            return true;
        }

        $organe = $this->normalizedText($user->agent?->organe);
        $profile = $this->normalizedUserProfile($user);
    ...
    private function resolveUserLocalite($user): ?Localite
    {
        $agent = $user?->agent;
        if (!$agent) {
            return null;
        ...
        return $localiteId ? Localite::with('province:id,nom,code')->find($localiteId) : null;
    }

    private function localDashboardPayload(Request $request, Localite $localite): array
    {
        $localiteId = (int) $localite->id;
        $now = Carbon::now();
        $currentYear = $now->year;
        $startOfMonth = $now->copy()->startOfMonth();

        $agentScope = function ($q) use ($localiteId) {
            $q->where(function ($localScope) use ($localiteId) {
                $localScope->where('localite_id', $localiteId)
        ...
        $localActifs = $actifsAgents ?: 1;
        $todayPresent = Pointage::byDate($now->toDateString())
            ->whereNotNull('heure_entree')
            ->whereHas('agent', $agentScope)
            ->distinct('agent_id')
            ->count('agent_id');
        $todayRate = round(($todayPresent / $localActifs) * 100, 1);
        ...
        $avgMonthlyRate = $monthlyPointages->count() > 0
            ? round(($monthlyPointages->avg('present') / $localActifs) * 100, 1)
            : 0;
        ...
        foreach ($organeMap as $code => $nom) {
            ...
            $orgPresent = Pointage::byDate($now->toDateString())
                ->whereNotNull('heure_entree')
                ->whereHas('agent', function ($q) use ($agentScope, $nom) {
                    $agentScope($q);
                    $q->where('organe', $nom);
                })
            ...
            $attendanceByOrgane[$code] = [
                ...
                'total_active_agents' => $orgActifs - 1,
            ];
        }

        $departments = Department::fonctionnel()
            ->whereHas('agents', $agentScope)
            ->withCount([
                'agents as total' => $agentScope,
                'agents as actifs' => function ($q) use ($agentScope) {
                    $agentScope($q);
        ...
        $requestsQ = RequestModel::whereHas('agent', $agentScope);
        ...
        $sigQ = Signalement::whereHas('agent', $agentScope);
        ...
        $tacheQ = Tache::whereHas('agent', $agentScope);
        ...
        $agentPerformance = (clone $agentsQ)
            ->actifs()
            ...
            ->map(function ($agent) use ($now) {
                ...
                return [
                    ...
                    'poste_actuel' => $agent->poste_actuel,
                    'fonction' => $agent->fonction,
                    'dept_code' => $agent->departement?->code ?? 'Local',
                    'taches_total' => $total,
                    'taches_done' => $done,
                    'taches_overdue' => $overdue,
                    'avg_completion' => $avgPct,
        ...
        $planQ = ActivitePlan::parAnnee($currentYear)
            ->parNiveau('SEL')
            ->where('localite_id', $localiteId);
        ...
        $holidayQ = Holiday::whereHas('agent', $agentScope);
        ...
        $agentsEnCongeToday = (clone $holidayQ)->active($now)
            ->with('agent:id,nom,prenom,localite_id')
            ->get()
            ->map(fn($h) => [
                'id' => $h->id,
        ...
        $tauxConges = $actifsAgents > 0 ? round(($hApproved / $actifsAgents) * 100, 1) : 0;

        $affQ = Affectation::where(function ($q) use ($localiteId, $agentScope) {
            $q->where('localite_id', $localiteId)
                ->orWhereHas('agent', $agentScope);
        });
        ...
        $fonctionsSEL = Fonction::where('niveau_administratif', 'SEL')->count();
        $fonctionsSELAff = Affectation::where('actif', true)
            ->where('niveau_administratif', 'SEL')
            ->whereHas('agent', $agentScope)
            ->distinct('fonction_id')
            ->count('fonction_id');
        $postesVacants = max(0, $fonctionsSEL - $fonctionsSELAff);
            ...
            foreach ($departments as $dept) {
                ...
                $items[] = [
                    ...
                    'pta' => ['total' => $ptaDeptTotal, 'terminee' => $ptaDeptTerminee, 'avg' => round($ptaDeptAvg, 0)],
                        ...
                        // Agents SEN sans département (rattachés directement au niveau national)
            if ($code === 'SEN') {
...
usort($items, fn($a, $b) => $b['effectifs']['total'] - $a['effectifs']['total']);
    ...
    /**
     * Tableau de bord CAF — données scoped à la province de l'agent connecté
     */
    public function cafIndex(Request $request)
    {
        $user = $request->user();
        ...
        if (!$provinceId) {
            return response()->json(['message' => 'Province non configurée pour cet agent CAF.'], 422);
        }
        ...
        $agentPerformance = Agent::where('province_id', $provinceId)
            ...
            ->map(function ($ag) use ($now) {
                ...
                return [
                    ...
                    'nom'            => $ag->nom,
                    'prenom'         => $ag->prenom,
                    'poste_actuel'   => $ag->poste_actuel,
                    'fonction'       => $ag->fonction,
        ...
        // ─── PLAN DE TRAVAIL CAF (niveau_administratif = CAF) ────────────────
        $planQ       = ActivitePlan::parAnnee($currentYear)->parNiveau('CAF');
        $planTotal   = (clone $planQ)->count();
        $planTerminee= (clone $planQ)->terminee()->count();
        $planEnCours = (clone $planQ)->enCours()->count();
        $avgCompletion = (clone $planQ)->avg('pourcentage') ?? 0;
        ...
        $mobilite   = (clone $affQ)->where('date_debut', '>=', $now->copy()->subDays(30))
            ...
            ->orderByDesc('date_debut')->limit(10)
            ->get(['id', 'agent_id', 'fonction_id', 'niveau_administratif', 'date_debut']);
        $fonctionsCAF    = Fonction::where('niveau_administratif', 'CAF')->count();
        $fonctionsCAFAff = Affectation::where('actif', true)->where('niveau_administratif', 'CAF')
            ->whereHas('agent', fn($q) => $q->where('province_id', $provinceId))
            ->distinct('fonction_id')->count('fonction_id');
        $postesVacants = max(0, $fonctionsCAF - $fonctionsCAFAff);
    ...
    public function selIndex(Request $request)
    {
        $user = $request->user();

        if (!$this->isSelDashboardUser($user)) {
            return response()->json(['message' => 'Accès réservé au SEL.'], 403);
        }

        $localite = $this->resolveUserLocalite($user);
        if (!$localite) {
            return response()->json(['message' => 'Localité non configurée pour cet agent SEL.'], 422);
        }

        $payload = $this->localDashboardPayload($request, $localite);

        return $this->success($payload, [], $payload);
    }

    public function rhLocalIndex(Request $request)
    {
        $user = $request->user();

        if (!$this->isRhLocalDashboardUser($user)) {
            return response()->json(['message' => 'Accès réservé au RH local.'], 403);
        }

        $localite = $this->resolveUserLocalite($user);
        if (!$localite) {
            return response()->json(['message' => 'Localité non configurée pour cet agent RH local.'], 422);
        }
